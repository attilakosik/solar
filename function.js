...
//GD start
jQuery(function ($) {
    $.each($('.ihc-theme_4 .ihc-user-list-label-result'), function () {
        if ($(this).html().indexOf("http") >= 0) {
            $(this).html('<a href="' + $(this).html() + '">' + $(this).html() + '</a>');
        }
    });

    /* $(document).on('click', '#ihc_submit_bttn', function () {
        $('html, body').animate({
            scrollTop: $(".regisztration-row").offset().top - 200
        }, 1000);
        //console.log($('.regisztration-row .ihc-register-notice').length);
    }); */

    if ($('.ihc-tos-wrap a').text() == 'Please accept our Terms of Use and Privacy Policy') {
        var res = $('.ihc-tos-wrap a').text().replace('Please', 'I').replace('our', 'the').replace('Terms of Use', '<a href="https://www.solarmaintenancemark.com/terms-of-use/">Terms of Use</a>').replace('Privacy Policy', '<a href="https://www.solarmaintenancemark.com/privacy-policy/">Privacy Policy</a>');

        $('.ihc-tos-wrap a').remove();
        $('.ihc-tos-wrap').append('<span>' + res + '</span>');
    }
});
